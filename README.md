...
* `LYNKS_ENDPOINT` – HTTP backend address
* `JANUS_WEBSOCKET_ENDPOINT` – Janus WebSocket endpoint
...
In main.py, the client is created like this:
```python
user = LynksUser("testuser", "test123", True)
```
The third parameter (sub_and_pub) controls whether the client will also subscribe to other participants’ feeds.
